...
class OrganizationCreateRequest(BaseModel):
    ...
    slug: Annotated[str, StringConstraints(min_length=3, max_length=120, pattern=r"^[a-z0-9-]+$")]
...
class InviteUserRequest(BaseModel):
    ...
    email: Annotated[
        str,
        StringConstraints(min_length=5, max_length=254, pattern=r"^[^@\s]+@[^@\s]+\.[^@\s]+$"),
    ]
...
class SafetyControlsUpsertRequest(BaseModel):
    ...
    blocked_categories: list[Annotated[str, StringConstraints(min_length=2, max_length=64)]] = Field(min_length=1, max_length=20)
