...
function useFocusScope(props, forwardedRef) {
  const {
    ...
    [setContainer]
  ), composedRefs = useComposedRefs(forwardedRef, setContainerTransition), focusScope = React.useRef({
    paused: !1,
    pause() {
      this.paused = !0;
    },
  ...
  React.useEffect(() => {
    ...
    function handleFocusIn(event) {
      ...
      container.contains(target) ? (target?.addEventListener("blur", handleBlur, { signal: controller.signal }), lastFocusedElementRef.current = target) : focus(lastFocusedElementRef.current);
    }
