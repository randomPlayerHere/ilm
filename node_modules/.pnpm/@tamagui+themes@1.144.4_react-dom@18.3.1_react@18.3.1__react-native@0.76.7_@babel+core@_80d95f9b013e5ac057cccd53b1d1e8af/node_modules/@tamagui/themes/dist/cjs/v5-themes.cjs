...
const blue = adjustPaletteLight("blue", import_colors.blue),
...
function adjustPaletteLight(name, palette) {
  const adjusted = {
    ...
    color11Key = Object.keys(palette).find(key => key.endsWith("11"));
  ...
  for (const key in adjusted) {
    const color = adjusted[key],
      ...
      newS = s * (isYellow ? 0.65 : 0.9),
      newL = Math.round(l * 0.982);
...
function adjustPaletteDark(name, palette) {
  ...
  for (const [index, key] of Object.keys(palette).entries()) {
    ...
    if (value?.startsWith("hsl(")) {
      ...
      if (number === 11) {
        const newL2 = Math.round(Math.min(100, l * 1.1)),
          newS2 = name === "yellowDark" ? 45 : 65;
        adjusted[key] = buildHSL(h, newS2, newL2);
        continue;
      }
      const newS = Math.round(s * (number >= 8 && number <= 10 ? 1 : isYellow ? 0.24 :
