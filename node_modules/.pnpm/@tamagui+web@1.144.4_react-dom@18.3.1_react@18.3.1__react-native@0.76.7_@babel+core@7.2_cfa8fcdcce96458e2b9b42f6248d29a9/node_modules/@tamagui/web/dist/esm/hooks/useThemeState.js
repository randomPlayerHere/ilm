...
const ThemeStateContext = createContext(""), allListeners = /* @__PURE__ */ new Map(), listenersByParent = {}, HasRenderedOnce = /* @__PURE__ */ new WeakMap(), HadTheme = /* @__PURE__ */ new WeakMap(), PendingUpdate = /* @__PURE__ */ new Map(), states = /* @__PURE__ */ new Map(), localStates = /* @__PURE__ */ new Map();
let shouldForce = !1;
...
const getRootThemeState = () => rootThemeState, useThemeState = (props, isRoot = !1, keys) => {
  ...
  const id = useId(), subscribe = useCallback(
    ...
    const needsUpdate = props.passThrough ? !1 : isRoot || props.name === "light" || props.name === "dark" || props.name === null ? !0 : HasRenderedOnce.get(keys) ? keys?.current?.size ? !0 : props.needsUpdate?.() : !0, [rerender, next] = getNextState(
      local,
      props,
      propsKey,
      isRoot,
  ...
  if (props.passThrough)
    return [!1, lastState || parentState || { name: "" }];
  ...
  if (name) {
    ...
    if (!found && !nameHasScheme && getScheme(parentName)) {
      ...
      outer: for (const base of potentialBases) {
        const candidates = [
          componentName ? `${base}_${name}_${componentName}` : void 0,
          `${base}_${name}`
        ].filter(Boolean);
